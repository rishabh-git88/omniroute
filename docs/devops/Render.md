...
| `SUPABASE_URL` | Server-only Supabase project URL for native Storage access |
| `SUPABASE_SERVICE_ROLE_KEY` | Server-only privileged Storage credential; never add it to Vercel |
| `SUPABASE_STORAGE_BUCKET` | Existing **private** bucket for original workspace source objects |
...
The API stores original TXT, Markdown, and PDF source bytes in a private native
Supabase Storage bucket and never returns public or presigned object URLs. The
server-only service-role key is used only inside the API after workspace
authorization; never create a public storage variable.

Set `FILES_STORAGE_DRIVER=supabase`, `SUPABASE_URL`,
`SUPABASE_SERVICE_ROLE_KEY`, and `SUPABASE_STORAGE_BUCKET` on the API. The
Blueprint sets `EMBEDDING_PROVIDER=disabled`
...
and frozen snapshots; Supabase Storage stores only the private source object.
...
Supabase Storage, embeddings, or chat providers.
