...
def classify_failure(event: ProviderEvent) -> str | None:
    ...
    if code in {"NETWORK_ERROR", "TIMEOUT", "HTTP_504"}:
        return "timeout"
    if code == "HTTP_429":
        return "rate_limit"
    if code in {"PROVIDER_UNAVAILABLE", "MODEL_UNAVAILABLE", "HTTP_404", "HTTP_503"}:
        return "unavailable"
...
class FallbackExecutor:
    ...
    def _validate_context(plans: list[ProviderExecutionPlan]) -> None:
        ...
        if any(plan.request.context_snapshot_id != source for plan in plans[1:]):
